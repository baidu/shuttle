...
private:
    ...
    File* fp_;
    // Generated by factory, so always valid
    OpenMode mode_;
    std::string path_;
...
bool SortFile::LoadIndexBlock(IndexBlock& index) {
    ...
    if (fp_->ReadAll(&index_offset, sizeof(index_offset)) < 0) {
        LOG(WARNING, "fail to read index offset: %s", path_.c_str());
        return false;
    }
    if (fp_->ReadAll(&magic_number, sizeof(magic_number)) < 0) {
    ...
    if (fp_->ReadAll(&index_size, sizeof(index_size)) < 0) {
    ...
    if (fp_->ReadAll(raw_buf, index_size) < index_size) {
...
bool SortFile::LoadDataBlock(DataBlock& block) {
    ...
    if (!fp_->ReadAll(&block_size, sizeof(block_size)) < 0) {
    ...
    if (fp_->ReadAll(raw_buf, block_size) < block_size) {
...
bool SortFile::FlushCurBlock() {
    ...
    WriteSerializedBlock(serialized);
